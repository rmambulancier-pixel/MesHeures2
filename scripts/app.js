...
function renderHome(){
  ...
  const diff=nDays(DB.s.anchor,now),qs=addD(DB.s.anchor,Math.floor(diff/14)*14),q=calcPer(qs,1).Q[0];
  const qG=calcPer(qs,1).G, monthG=calcPer(m+'-01',1).G, br=brutOf(qG), monthBr=brutOf(monthG);
  const N=DB.s.base*120,pc=Math.min(100,q.seuil/N*100);
  ...
  $('homeKpi').innerHTML=[
    ...
    ['Paniers',month.ir+month.iru,''],['Alertes',month.alerts.length,month.hard?'bad':'']
  ...
  $('homePay').innerHTML=`<div class="home-value">${EUR(monthBr.tot)}</div><div class="home-muted">Brut estimé du mois · quatorzaine courante : ${EUR(br.tot)}</div><div class="home-list"><div class="home-row"><span>TTE mois</span><b>${F(month.tte)}</b></div><div class="home-row"><span>Nuit</span><b>${C2(month.nuit)} h</b></div><div class="home-row"><span>IDAJ</span><b>${C2(month.idaj)} h</b></div></div>`;
  const sorted=month.alerts.slice().sort((a,b)=>(a.lvl==='b'?0:1)-(b.lvl==='b'?0:1)).slice(0,6);
  $('homeAlertCount').textContent=month.alerts.length?`${month.hard} critique(s) · ${month.warn} attention(s)`:'aucune';
  $('homeAlerts').innerHTML=sorted.length?sorted.map(a=>`<div class="al ${a.lvl}" onclick="mhOpenDay('${a.k}')" style="cursor:pointer"><b>${shortY(a.k)}</b> — ${esc(a.m)}</div>`).join(''):'<div class="al k">✅ Rien de critique détecté ce mois-ci.</div>';
}
...
const _renderPayOriginal=renderPay;
function renderPay(){
  _renderPayOriginal();
...
const _renderAuditOriginal=renderAudit;
function renderAudit(){
  _renderAuditOriginal();
...
const _renderRegOriginal=renderReg;
function renderReg(){
  _renderRegOriginal();
...
/* Version et cache */
if($('mhVersion'))$('mhVersion').textContent='V15';
