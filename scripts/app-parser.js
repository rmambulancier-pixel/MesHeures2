// scripts/app-parser.js — Moteur de lecture et d'analyse : AmbuTrack, Bulletins de paie & ROMI1

// Configuration des workers et expressions régulières
const PDF_WORKER = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/legacy/build/pdf.worker.min.js';
let _pdfWorkerSet = false;

const RX_PER = /P[ée]riode\s+du\s+(\d{1,2})\/(\d{1,2})\/(\d{4})\s+au\s+(\d{1,2})\/(\d{1,2})\/(\d{4})/gi;
const RX_DAY = /(\d{1,2})\s+([A-Za-zÀ-ÿ]{3,})\s+(.*)/gi;
const RX_HOR = /(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})/;
const RX_PZ  = /(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})\s*(EXT|ENT)?/gi;

const RM_FIELDS = [
  ['tteH', 'TTE période (h)'],
  ['joursPresence', 'Jours de présence'],
  ['rcSolde', 'Solde RC cumulé (h)'],
  ['rcAcquis', 'RC acquis ce mois (h)'],
  ['rcAvant', 'RC antérieur (h)'],
  ['hs125EligH', 'HS 25 % éligibles (h)'],
  ['hs125PayeesH', 'HS 25 % payées (h)'],
  ['hs150EligH', 'HS 50 % éligibles (h)'],
  ['hs150PayeesH', 'HS 50 % payées (h)'],
  ['primeAncPct', 'Prime ancienneté (%)'],
  ['habillageMontant', 'Habillage brut (€)'],
  ['indemniteRepasN', 'Paniers IR ext. (nb)'],
  ['indemniteRepasUniqueN', 'Paniers IRU int. (nb)']
];

let romiTmp = { _loaded: true };

// Accesseurs de sécurisation de données
function gd(dateKey) {
  if (!DB.days[dateKey]) {
    DB.days[dateKey] = { t: 'REPOS', p: [] };
  }
  return DB.days[dateKey];
}

function gb(periodStart) {
  if (!DB.bul) DB.bul = {};
  if (!DB.bul[periodStart]) {
    DB.bul[periodStart] = { p25: 0, p50: 0, rcOld: 0, rcAcq: 0 };
  }
  return DB.bul[periodStart];
}

function setBul(field, val) {
  const B = gb(DB.per.start);
...
function cap(v, max) {
  return v != null && v <= max ? v : undefined;
}

// ==================== 1. IMPORT AMBUTRACK (PDF & TEXTE) ====================
...
function parsePdfFile(file) {
  ...
  if (!lib) {
    out.innerHTML = '<div class="al b">❌ PDF.js non chargé. Recharge la page en ligne.</div>';
    return;
  }
  if (!_pdfWorkerSet) {
    ...
    try {
      ...
      for (let p = 1; p <= pdf.numPages; p++) {
        ...
        ct.items.forEach(it => {
          ...
          if (py != null && Math.abs(y - py) > 2) {
            text += line + '\n';
            line = '';
          } else if (px != null && x - px > (it.height || 10) * 0.22) {
            line += ' ';
          }
          line += it.str;
          py = y;
      ...
      if (!text.trim()) {
        out.innerHTML = '<div class="al b">❌ Aucun texte extrait du fichier.</div>';
        return;
      }
      $('impTxt').value = text;
      parsePDF();
    } catch (err) {
      out.innerHTML = '<div class="al b">❌ Erreur de lecture : ' + esc(err.message) + '</div>';
    }
  };
  reader.onerror = () => {
    out.innerHTML = '<div class="al b">❌ Fichier illisible.</div>';
  };
...
function parsePDF() {
  const out = $('impOut');
  const raw = $('impTxt').value;
  if (!raw.trim()) {
    out.innerHTML = '<div class="al w">Colle d\'abord le texte à analyser.</div>';
    return;
  }
  ...
  if (!marks.length) {
    out.innerHTML = '<div class="al b">❌ Ligne « Période du … au … » introuvable.</div>';
    return;
  }
  if (typeof pushUndo === 'function') pushUndo('Import PDF AmbuTrack');

  let html = '', tot = 0, allFer = [];
  marks.forEach((m, i) => {
    const blk = t.slice(m.index, i + 1 < marks.length ? marks[i + 1].index : t.length);
    const st = m[3] + '-' + pad(+m[2]) + '-' + pad(+m[1]);
    const en = m[6] + '-' + pad(+m[5]) + '-' + pad(+m[4]);
    const R = parseBlock(blk, st, en);
    tot += R.n;
    R.fer.forEach(k => allFer.push(k));
  ...
  if (tot) {
    curDate = marks[0][3] + '-' + pad(+marks[0][2]) + '-' + pad(+marks[0][1]);
...
function parseBlock(b, st, en) {
  RX_DAY.lastIndex = 0;
  const rows = [...b.matchAll(RX_DAY)];
  const rep = { T: 0, REPOS: 0, RC: 0, CP: 0, MAL: 0 };
  const fer = [], warn = [];
  let cur = st, n = 0;
  ...
  rows.forEach(r => {
    const dn = +r[1];
    let g = 0;
    while (+cur.slice(8) !== dn && g < 45) {
      cur = addD(cur, 1);
      g++;
    }
    if (g >= 45) {
      warn.push('jour ' + dn + ' non replacé');
      return;
    }
    const k = cur;
    const txt = r[3];
    ...
    if (ty === 'T') {
      RX_HOR.lastIndex = 0;
      const hh = txt.match(RX_HOR);
      if (hh) {
        d.deb = hh[1];
        d.fin = hh[2];
      } else {
        warn.push(short(k) + ' horaires illisibles');
      }
      RX_PZ.lastIndex = 0;
      [...txt.matchAll(RX_PZ)].forEach(p => d.p.push({ d: p[1], f: p[2], ty: p[3] || 'ENT' }));
      if (isFerie(k)) {
        d.fer = true;
        fer.push(k);
      }
  ...
  if (n) {
    const days = nDays(st, en) + 1;
    const nb = Math.max(1, Math.round(days / 14));
    const ex = DB.periods.find(p => p.start === st);
    if (ex) ex.nb = nb;
    else DB.periods.push({ start: st, nb });
...
function parseExcel(file) {
  ...
  if (typeof XLSX === 'undefined') {
    out.innerHTML = '<div class="al b">❌ Bibliothèque XLSX non chargée.</div>';
    return;
  }
  const reader = new FileReader();
  reader.onload = function(e) {
    ...
    try {
      ...
      out.innerHTML = '<div class="al b">❌ Fichier Excel invalide.</div>';
      return;
    }
    ...
    if (!rows.length) {
      out.innerHTML = '<div class="al b">❌ Feuille de données vide.</div>';
      return;
    }

    const S = DB.s;
    const pdMin = P(S.panDeb), pfMin = P(S.panFin);
    const rep = { T: 0, REPOS: 0, RC: 0, CP: 0, MAL: 0 };
    const fer = [], warnNuit = [], warnH = [], dates = [];
    let iru = 0, n = 0;

    if (typeof pushUndo === 'function') pushUndo('Import Excel');

    rows.forEach(row => {
      const k = excelDate(row['Date']);
      if (!k) return;
      ...
      if (ty === 'T') {
        ...
        const deb = String(row['Début'] || '').trim();
        const fin = String(row['Fin'] || '').trim();

        if (nuit && deb === '00:00' && fin === '00:00') {
          warnNuit.push(k);
          return;
        }
        if (!/^\d{1,2}:\d{2}$/.test(deb) || !/^\d{1,2}:\d{2}$/.test(fin)) {
          warnH.push(k);
          return;
        }
        d.deb = deb;
        ...
        if (pz && pz !== '-') {
          ...
          if (pm) {
            const pS = pad(+pm[1]) + ':' + pm[2];
            const pE = pad(+pm[3]) + ':' + pm[4];
            d.p.push({ d: pS, f: pE, ty: 'ENT' });
            if (panOK) iru++;
          }
        }
        if (isFerie(k)) {
    ...
    if (!n) {
      out.innerHTML = '<div class="al b">❌ Aucune journée exploitable dans le fichier.</div>';
      return;
    }

    dates.sort();
    const first = dates[0], last = dates[dates.length - 1];
    let qs = addD(S.anchor, Math.floor(nDays(S.anchor, first) / 14) * 14);

    while (qs <= last) {
      if (!DB.periods.some(p => p.start === qs)) {
        DB.periods.push({ start: qs, nb: 1 });
      }
      gb(qs);
      qs = addD(qs, 14);
    }
    DB.periods.sort((a, b) => (a.start < b.start ? -1 : 1));
    DB.per = { start: DB.periods[DB.periods.length - 1].start, nb: 1 };
    save();

    out.innerHTML = `<div class="al k"><b>✅ ${n} journées importées</b> du ${shortY(first)} au ${shortY(last)}</div>` +
      (iru ? `<div class="al w">🍽️ ${iru} pauses assignées en IRU par défaut.</div>` : '') +
      (fer.length ? '<div class="al w">☀️ Fériés : ' + fer.map(shortY).join(', ') + '</div>' : '') +
      (warnNuit.length ? '<div class="al b">🌙 ' + warnNuit.length + ' nuit(s) 00:00-00:00 à vérifier.</div>' : '') +
      (warnH.length ? '<div class="al w">⚠️ Horaires non reconnus : ' + warnH.map(shortY).join(', ') + '</div>' : '');

    curDate = last;
    curMonth = last.slice(0, 7);
    if (typeof renderAll === 'function') renderAll();
  };
  reader.readAsArrayBuffer(file);
  $('fx').value = '';
...
function parseBulFile(file) {
  ...
  reader.onload = async e => {
    try {
      ...
      out.innerHTML = '<div class="al b">❌ Erreur lecture PDF : ' + esc(err.message) + '</div>';
    }
  };
  reader.onerror = () => {
    out.innerHTML = '<div class="al b">❌ Fichier illisible.</div>';
...
function parseBulletin() {
  ...
  if (!raw.trim()) {
    $('bulOut').innerHTML = '<div class="al w">Colle d\'abord le texte du bulletin.</div>';
    return;
  }
  ...
  for (const l of lines) {
    if (/Monsieur|Madame/i.test(l)) {
      bul.nom = l.replace(/Monsieur|Madame/gi, '').trim();
      break;
    }
  }
  ...
  for (const l of lines) {
    ...
    if (/salaire de base/i.test(l) && ns.length >= 3) {
      ...
      const ns2 = numsOfLine(l.replace(/25\s*%/gi, ''));
      if (ns2.length >= 3) {
        bul.hs25H = ns2[0];
        bul.hs25T = ns2[1];
        bul.hs25M = ns2[ns2.length - 1];
      }
    } else if (/suppl[ée]?mentaires?\s*50/i.test(l)) {
      const ns2 = numsOfLine(l.replace(/50\s*%/gi, ''));
      if (ns2.length >= 3) {
        bul.hs50H = ns2[0];
        bul.hs50T = ns2[1];
        bul.hs50M = ns2[ns2.length - 1];
      }
  ...
  if (existing >= 0) DB.bulletins[existing] = bul;
  else DB.bulletins.push(bul);
...
function afficherBulletin(bul) {
  ...
  let calcTTE = 0, calcH25 = 0, calcH50 = 0, calcIR = 0, calcIRU = 0, calcIDAJ = 0;
  let calcHab = 0, calcFer = 0, calcTrav = 0, calcDim = 0, calcRC = 0;
  const mKey = bul.mois;
  ...
  addRow('Heures normales', bul.hNorm, calcNorm, 'h', 'Base contractuelle + équivalences');
  if (bul.hs25H != null) addRow('HS 25%', bul.hs25H, calcH25 / 60, 'h', 'Heures au-delà du seuil de base');
  if (bul.hs50H != null) addRow('HS 50%', bul.hs50H, calcH50 / 60, 'h', 'Heures au-delà du contingent 25%');
  ...
  if (bul.irN != null)   addRow('Paniers IR (ext.)', bul.irN, calcIR, 'repas', 'Indemnité repas extérieur');
  if (bul.iruN != null)  addRow('Paniers IRU (int.)', bul.iruN, calcIRU, 'repas', 'Indemnité repas unique');
  if (bul.taux != null)  addRow('Taux horaire', bul.taux, S.taux, '€/h', '');

  if (bul.brut != null) {
    const anc = calcAnc(S.emb || DEF.emb);
    ...
    addRow('Salaire brut', bul.brut, calcBrut, '€', ancPct ? `Avec prime ancienneté ${ancPct}%` : 'Sans ancienneté');
  }

  if (bul.net != null) {
    const calcNet = (bul.brut || 0) * S.net + calcIR * S.ir + calcIRU * S.irT;
    addRow('Net estimé', bul.net, calcNet, '€', 'Brut × ' + S.net + ' + indemnités repas');
  }
...
<td>${r.ok ? '✅' : '🚩'}</td>
  ...
  if (anomalies.length === 0) {
    al = '<div class="al k">✅ Aucun écart significatif — conformité validée.</div>';
  } else {
...
👤 Ancienneté bulletin : <b>${bul.ancienneteTxt}</b> — Calculée : <b>${ancCalc.y} ans ${ancCalc.m} mois</b> (taux conventionnel : <b>${ancCalc.pct}%</b>)
</div>`;
  }

  if (bul.rcAvant != null && bul.rcApres != null) {
    al += `<div class="al i">⏱️ RC bulletin : début <b>${bul.rcAvant.toFixed(2)}h</b> → fin <b>${bul.rcApres.toFixed(2)}h</b> (acquis : <b>${(bul.rcApres - bul.rcAvant).toFixed(2)}h</b>)</div>`;
  }
...
function extractRomiFields(raw) {
  ...
  const setCapped = (obj, key, val, max) => {
    const c = cap(val, max);
    if (c != null) obj[key] = c;
    else rejected++;
  };
  ...
  for (const l of lines) {
    ...
    if (/T\.?\s*T\.?\s*E\.?/i.test(l) && noSlash && ns.length >= 2) {
      setCapped(f, 'tteH', ns[0], 400);
      setCapped(f, 'tteAnneeH', ns[1], 3000);
    } else if (/jours?\s+de\s+pr[eé]sence/i.test(l) && ns.length >= 2) {
      setCapped(f, 'joursPresence', ns[0], 31);
      setCapped(f, 'joursAnnee', ns[1], 366);
    } else if (/heures?\s+suppl[eé]mentaires?/i.test(l) && noSlash && !/125|150/.test(l) && ns.length >= 2) {
      setCapped(f, 'hsPeriodeH', ns[0], 200);
      setCapped(f, 'hsAnneeH', ns[1], 1000);
    } else if (/ann[eé]e\s*N-1/i.test(l) && ns.length >= 1) {
      setCapped(f, 'cpN1Solde', ns[0], 60);
      ...
      setCapped(f, 'rcAvant', ns[0], 1000);
      setCapped(f, 'rcAcquis', ns[1], 200);
      setCapped(f, 'rcSolde', ns[ns.length - 1], 1000);
    } else if (/salaire\s+de\s+base/i.test(l) && ns.length >= 1) {
      setCapped(f, 'salaireBaseH', ns[0], 400);
    } else if (/amplitude/i.test(l) && ns.length >= 1) {
      setCapped(f, 'depAmplH', ns[0], 100);
    } else if (/125\s*%/.test(l) && ns.length >= 1) {
      setCapped(f, 'hs125EligH', ns[0], 200);
      if (ns.length >= 2) setCapped(f, 'hs125PayeesH', ns[1], 200);
    } else if (/150\s*%/.test(l) && ns.length >= 1) {
      setCapped(f, 'hs150EligH', ns[0], 200);
      if (ns.length >= 2) setCapped(f, 'hs150PayeesH', ns[1], 200);
    } else if (/anciennet[eé]/i.test(l) && ns.length >= 1) {
      setCapped(f, 'primeAncPct', ns[0], 20);
    } else if (/habillage/i.test(l) && ns.length >= 1) {
      setCapped(f, 'habillageMontant', ns[ns.length - 1], 500);
    } else if (/repas\s+unique/i.test(l) && ns.length >= 1) {
      setCapped(f, 'indemniteRepasUniqueN', ns[0], 31);
    } else if (/indemnit[eé]\s+de\s+repas/i.test(l) && !/unique/i.test(l) && ns.length >= 1) {
      setCapped(f, 'indemniteRepasN', ns[0], 31);
    }
  }
...
function parseRomiFile(file) {
  const out = $('romiOcrOut');
  if (!file) return;
  if (typeof Tesseract === 'undefined') {
    out.innerHTML = '<div class="al b">❌ Moteur OCR Tesseract non chargé.</div>';
    return;
  }
  out.innerHTML = '<div class="al i">⏳ Traitement OCR en cours… Cela peut prendre 10 à 20 secondes.</div>';

  Tesseract.recognize(file, 'fra')
    .then(({ data: { text } }) => {
      $('romiRawTxt').value = text;
      const extracted = extractRomiFields(text);
      romiTmp = { ...extracted, _loaded: true };
      out.innerHTML = '<div class="al k">✅ Extraction OCR terminée. Vérifie les valeurs extraites ci-dessous.</div>';
      renderRomiTab();
    })
    .catch(err => {
      out.innerHTML = '<div class="al b">❌ Échec du traitement OCR : ' + esc(err.message) + '</div>';
    });
  $('fromi').value = '';
}
...
function renderRomiTab() {
  ...
  if (!romiTmp._loaded && last) {
    romiTmp = { ...last, _loaded: true };
  }
...
function saveRomi() {
  ...
  if (!start || !end) {
    $('romiOut').innerHTML = '<div class="al w">Renseigne au moins les dates de début et de fin.</div>';
    return;
  }
  const entry = { ...romiTmp, start, end, importedAt: new Date().toLocaleDateString('fr-FR') };
  delete entry._loaded;
  delete entry._rejected;
...
function afficherRomi(e) {
  ...
  const rcNonPaye =
    Math.max((e.hs125EligH || 0) - (e.hs125PayeesH || 0), 0) +
    Math.max((e.hs150EligH || 0) - (e.hs150PayeesH || 0), 0);
  ...
  let al = '';
  if (rcNonPaye > 0.05) {
    al += `<div class="al b">🚩 <b>${rcNonPaye.toFixed(2)} h</b> d'heures supplémentaires converties en RC au lieu d'être payées.
<br><span class="mut">CCN IDCC 16 (art. 3.2 accord-cadre) : cette conversion requiert l'accord ou la demande expresse du salarié.</span></div>`;
  } else {
    al += '<div class="al k">✅ Aucune conversion forcée HS → RC relevée.</div>';
  }
  ...
  if (idx > 0) {
    ...
    if (prev.rcSolde != null && e.rcAvant != null) {
      ...
      if (Math.abs(ec) > 0.1) {
        al += `<div class="al w">⚠️ Incohérence solde antérieur : ${e.rcAvant.toFixed(2)} h vs ${prev.rcSolde.toFixed(2)} h précédent (écart de ${ec.toFixed(2)} h).</div>`;
      }
    }
  }
  ...
  if (e.start && e.end) {
    ...
    if (calcTte > 0 && e.tteH) {
      ...
      al += `<div class="al ${Math.abs(ec) < 0.3 ? 'k' : 'i'}">${Math.abs(ec) < 0.3 ? '✅' : 'ℹ️'} TTE application sur la période : <b>${F(calcTte)}</b> vs ROMI1 : <b>${e.tteH.toFixed(2)} h</b>${Math.abs(ec) >= 0.3 ? ' — écart ' + ec.toFixed(2) + ' h' : ''}</div>`;
    }
  }
...
function renderRomiHist() {
  const keys = Object.keys(DB.romi).sort();
  if (!keys.length) {
    $('romiHist').innerHTML = '<div class="al i">Aucun relevé ROMI1 enregistré pour l\'instant.</div>';
    return;
  }
